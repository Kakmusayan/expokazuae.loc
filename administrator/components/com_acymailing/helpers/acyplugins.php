	...
	function wrapText($text, $tag)
	{
		...
		for($i=0 ; $i<$numChar ; $i++)
		{
			if($newText[$i] == '<')
			{
				...
				{
					if($numChar >= ($i+strlen($oneAllowedTag)+1) && substr($newText, $i, strlen($oneAllowedTag)+1) == '<'.$oneAllowedTag && ($newText[$i+strlen($oneAllowedTag)+1] == ' ' || $newText[$i+strlen($oneAllowedTag)+1] == '>'))
					{
						$write = false;
						$open[] = '</'.$oneAllowedTag.'>';
					}
				...
				foreach($aloneAllowedTags as $oneAloneAllowedTag)
				{
					if($numChar >= ($i+strlen($oneAllowedTag)+1) && substr($newText, $i, strlen($oneAllowedTag)+1) == '<'.$oneAllowedTag && ($newText[$i+strlen($oneAllowedTag)+1] == ' ' || $newText[$i+strlen($oneAllowedTag)+1] == '>'))
					{
						$write = false;
		...
		if(substr($newText, 0, 5) == '<br/>')
			$newText = substr($newText, 5, strlen($newText)-5);

		if(substr($newText, strlen($newText)-5, strlen($newText)) == '<br/>')
			$newText = substr($newText, 0, strlen($newText)-5);
