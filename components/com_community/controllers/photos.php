	...
	public function regen()
	{
                ...
                if(!empty($data) && $start == 's')
                {
                    ...
                    foreach($photos as $pic)
                    {
                            ...
                            //{
                                    $srcPath	= JPATH_ROOT . DS . CString::str_ireplace( JPATH_ROOT . DS , 
'' , $pic->image );
                                    $destPath	= JPATH_ROOT . DS . CString::str_ireplace( JPATH_ROOT . DS , 
'' , $pic->thumbnail );
                                    ...
                                    if(JFile::exists($srcPath) && !JFile::exists($destPath))
                                    {
                                            ...
                                            CImageHelper::createThumb($srcPath, $destPath, $destType, 128, 
128);
                                            $msg[]= "Regenerate thumbnails for ". $srcPath. '<br/>';
                                    }
                                    else
                                    {
                                            $originalPath = JPATH_ROOT . DS . CString::str_ireplace( 
JPATH_ROOT . DS , '' , $pic->original );

                                            if(JFile::exists($originalPath) && !JFile::exists($destPath))
                                            {
                                                $info			= getimagesize( JPATH_ROOT . DS . 
$pic->original );
                                                $destType                   = 
image_type_to_mime_type($info[2]);

                                                CImageHelper::createThumb($originalPath, $destPath, 
$destType, 128, 128);
	...
	public function jsonupload()
	{
        ...
        private function _outputJSONText( $hasError, $text, $thumbUrl = null, $albumId = null, $photoId = 
null )
	{
	...
	private function _showUploadError( $hasError , $message , $thumbUrl = null, $albumId = null, $photoId 
= null )
	{ 
			$this->_outputJSONText( $hasError , $message , $thumbUrl, $albumId, $photoId );
	}

	private function _addActivity( $command, $actor , $target , $title , $content , $app , $cid, $group, 
$event, $param='')
	{
	...
	private function _saveAlbum( $id = null, $albumName = '' )
	{
		...
		if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
		{
			...
			if(is_null($id) && $album->groupid != 0){
				...
				$params->set('group_url' , 
'index.php?option=com_community&view=groups&task=viewgroup&groupid='.$group->id );
				$params->set('album' , $album->name );
				$params->set('album_url' , 
'index.php?option=com_community&view=photos&task=album&albumid='.$album->id.'&groupid='.$group->id );
				$params->set( 'url', 
'index.php?option=com_community&view=photos&task=album&albumid='.$album->id);
				CNotificationLibrary::add( 'groups_create_album' , $my->id , $groupMembers , 
JText::sprintf('COM_COMMUNITY_GROUP_NEW_ALBUM_NOTIFICATION') , '' , 'groups.album' , $params);
	...
	private function _storeOriginal($tmpPath, $destPath, $albumId = 0)
	{
		...
		$originalPathFolder	= JPATH_ROOT . DS . $config->getString('photofolder') . DS . 
JPath::clean( $config->get('originalphotopath') );
		$originalPathFolder	= $originalPathFolder . DS . $my->id . $albumPath;
		
		if( !JFile::exists( $originalPathFolder ) )
		{
			JFolder::create( $originalPathFolder, (int) octdec( 
$config->get('folderpermissionsphoto') ) );
			JFile::copy( JPATH_ROOT . DS . 'components' . DS . 'com_community' . DS . 
'index.html' , $originalPathFolder . DS . 'index.html' );
		}

		if(!JFile::copy( $tmpPath, $destPath ) )
		{
			JError::raiseWarning(21, JText::sprintf('COM_COMMUNITY_ERROR_MOVING_UPLOADED_FILE' , 
$destPath));
		}
	}
	...
	public function ajaxLinkToProfile( $photoId )
	{
		...
		$message	= '<form name="change-profile-picture" id="change-profile-picture" 
method="post" action="' . 
CRoute::_('index.php?option=com_community&view=profile&task=linkPhoto&userid='.$my->id) . '">'
					. JText::_('COM_COMMUNITY_PHOTOS_SET_AVATAR_DESC')
					. '<input type="hidden" name="id" value="' . $photoId . '" />'
					. '</form>';

		$actions  = '<button  class="button" 
onclick="joms.jQuery(\'#change-profile-picture\').submit()">' . JText::_('COM_COMMUNITY_YES') . '</button>';
		$actions .= '&nbsp;<button class="button" onclick="cWindowHide();">' . 
JText::_('COM_COMMUNITY_NO') . '</button>';
	...
	public function ajaxAddPhotoTag($photoId, $userId, $posX, $posY, $w, $h)	
	{
		...
		if($tagId > 0)
		{
			...
			$jsonString = '{'
			 	...
			 	. 'profileUrl:\'' . 
CRoute::_('index.php?option=com_community&view=profile&userid='.$userId, false) . '\','
			 	. 'top:' . $posX . ','
			 	. 'left:' . $posY . ','
			 	. 'width:' . $w . ','
			 	. 'height:' . $h . ','
			...
			if($my->id != $userId)
			{
				...
				CNotificationLibrary::add( 'photos_tagging' , $my->id , $userId , 
JText::sprintf( 'COM_COMMUNITY_SOMEONE_TAG_YOU' ) , '' , 'photos.tagging' , $params );
			}				 
		...
		else
		{
			...
			$actions = '<button class="button" 
onclick="cWindowHide();joms.gallery.cancelNewPhotoTag();" name="close">' . 
JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . '</button>';
			...
			$response->addScriptCall('cWindowShow', '', JText::_('COM_COMMUNITY_NOTICE'), 450, 
200, 'warning');
...
200, 'warning');
	...
	public function ajaxDisplayCreator($photoid)
	{
		...
		$html = JText::sprintf('COM_COMMUNITY_UPLOADED_BY', 
...
$photoCreator->getDisplayName());
    ...
    public function ajaxRemoveFeatured( $albumId )
    {
		...
		$actions = '<input type="button" class="button" onclick="window.location.reload();" value="' 
. JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . '"/>';
...
. JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . '"/>';
	...
	{	
		...
		if( !$config->get('enablereporting') || ( ( $my->id == 0 ) && ( 
!$config->get('enableguestreporting') ) ) )
		{
			return '';
		}
...
// 		return JText::_('COM_COMMUNITY_PHOTO_DELETED');
	...
	public function ajaxSetDefaultPhoto( $albumId , $photoId )
	{
		...
		{
			$response->addScriptCall( 'alert' , 
JText::_('COM_COMMUNITY_PERMISSION_DENIED_WARNING') );
			return $response->sendResponse();
		}
	...
	public function ajaxRemoveAlbum( $id , $currentTask )
	{
		...
		$actions	= '<form name="jsform-photos-ajaxRemoveAlbum" method="post" action="' . 
CRoute::_('index.php?option=com_community&view=photos&task=removealbum') . '">';
		$actions	.= '<input type="submit" value="' . JText::_('COM_COMMUNITY_YES_BUTTON') . '" 
class="button" name="Submit"/>';
		...
		$actions	.= '&nbsp;';
		$actions	.= '<input onclick="cWindowHide();" type="button" value="' . 
JText::_('COM_COMMUNITY_NO_BUTTON') . '" class="button" />';
...
JText::_('COM_COMMUNITY_PERMISSION_DENIED_WARNING') );
		...
		$actions  =	'<button  class="button" onclick="joms.gallery.removePhoto(\'' . $photoId . 
'\',\'' . $action . '\',\'' . $updatePlayList . '\');">' . JText::_('COM_COMMUNITY_YES_BUTTON') . 
'</button>';

		$actions .=	'&nbsp;<button class="button" onclick="cWindowHide();return false;">' . 
JText::_('COM_COMMUNITY_NO_BUTTON') . '</button>';

		$response->addAssign('cwin_logo', 'innerHTML', 
JText::_('COM_COMMUNITY_PHOTOS_REMOVE_PHOTO_BUTTON') );
		$response->addScriptCall('cWindowAddContent', $html, $actions);
...
JText::_('COM_COMMUNITY_PERMISSION_DENIED_WARNING'), $actions );
			$response->sendResponse();
		}
	...
	public function showWallContents($photoId)
	{
		...
		CFactory::load( 'helpers' , 'owner' );
		$contents	= CWallLibrary::getWallContents( 'photos' , $photoId , ($my->id == 
$photo->creator || COwnerHelper::isCommunityAdmin() ) , $limit , 0 , 'wall.content' , 'photos,photo' );
	...
	public function ajaxSaveCaption($photoId , $caption, $needAddScript = true)
	{
		...
		{
			$response->addScriptCall( 'alert' , 
JText::_('COM_COMMUNITY_PHOTOS_NOT_ALLOWED_EDIT_CAPTION_ERROR') );
			return $response->sendResponse();
		}
		...
		{
			$response->addScriptCall('joms.gallery.updatePhotoCaption', $photo->id, 
$photo->caption);
		}
	...
	public function ajaxSaveAlbumDesc($albumId , $description, $needAddScript = true)
	{
		...
		{
			$response->addScriptCall( 'alert' , 
JText::_('COM_COMMUNITY_PHOTOS_NOT_ALLOWED_EDIT_ALBUM_DESC_ERROR') );
			return $response->sendResponse();
		}
		...
		if ($needAddScript === true)
		{
			$response->addScriptCall('joms.jQuery(".community-photo-desc-editable").val', 
$album->description);
		}
	...
	public function ajaxSwitchPhotoTrigger( $photoId )
	{
		...
		$creatorHTML	= JText::sprintf('COM_COMMUNITY_UPLOADED_BY', 
CRoute::_('index.php?option=com_community&view=profile&userid='.$creator->id), $creator->getDisplayName());
		...
		if($config->get('tags_photos')){
			...
			$tagsHTML = $tags->getHTML('photos', $photoId, $photo->creator == $my->id || 
COwnerHelper::isCommunityAdmin());
		}*/

		$reportHTML	= $report->getReportingHTML( JText::_('COM_COMMUNITY_REPORT_BAD_PHOTO') , 
'photos,reportPhoto' , array( $photoId ) );
	...
	public function ajaxUpdateCounter( $albumId )
	{
		...
		if($photoUploaded/$photoUploadLimit>=COMMUNITY_SHOW_LIMIT) {
		    $response->addScriptCall('joms.jQuery("#photoUploadedCounter").html("' . 
JText::sprintf('COM_COMMUNITY_UPLOAD_LIMIT_STATUS', $photoUploaded, $photoUploadLimit ) .'")');
		}
	...
	public function ajaxGetAlbumURL( $albumId, $groupId = '' )
	{
		...
		$albumURL	= 
CRoute::_('index.php?option=com_community&view=photos&task=album&albumid='.$albumId.''.$groupURL, false);
	...
	public function ajaxGotoOldUpload( $albumId, $groupId = '' )
	{
		$my			=&	CFactory::getUser();
		$albumURL	= 
CRoute::_('index.php?option=com_community&view=photos&task=uploader&userid='.$my->id, false);

                if($groupId!='')
                {
                    $albumURL   = 
CRoute::_('index.php?option=com_community&view=photos&task=uploader&groupid=' . $groupId . '&albumid=' . 
$albumId, false);
                }

		$msg = JText::_('COM_COMMUNITY_PHOTOS_STATUS_BROWSER_NOT_SUPPORTED_ERROR');
		$action = '<script type="text/javascript">joms.jQuery("#cwin_close_btn").click(function() { 
joms.photos.multiUpload.goToOldUpload(\''.$albumURL.'\') });</script><button class="button" 
onclick="joms.photos.multiUpload.goToOldUpload(\''.$albumURL.'\')">'.JText::_('COM_COMMUNITY_OK_BUTTON').'</button>';
		$objResponse	=	new JAXResponse();
	...
	{		 		
		...
		if ($albumName == '')
		{
			...
			if (empty($album))
			{
				...
				$album->creator = $my->id;
				$album->created = gmdate('Y-m-d H:i:s');
				$album->name    = JText::sprintf('COM_COMMUNITY_DEFAULT_ALBUM_CAPTION', 
$my->getDisplayName());
				$album->type    = $handler->getType();
				$album->default = '1';
				...
				$album->store();		
				$objResponse->addScriptCall( 'joms.photos.multiUpload.assignNewAlbum', 
$album->id, $album->name );	
			}
			else
			{
...
JText::_('COM_COMMUNITY_PHOTOS_STATUS_UNABLE_SAVE_ALBUM_ERROR') );
				...
				$objResponse->addScriptCall( 'joms.photos.multiUpload.assignNewAlbum', 
$album->id, $albumName );
				//$objResponse->addScriptCall( 'joms.photos.multiUpload.startUploading', 
$album->id );
			}
		}
	...
	{	
		...
		$label = array(
						'filename'			=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_FILENAME"),
						'size'				=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_SIZE"),
						'status'			=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_STATUS"),
						'filedrag'			=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_DRAG_FILES"),
						'addfiles'			=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_ADD_FILES"),
						'startupload'		=> 
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_START_UPLOAD"),
						'invalidfiletype'	=> 
JText::_("COM_COMMUNITY_PHOTOS_INVALID_FILE_ERROR"),
						'exceedfilesize'	=> 
...
JText::_("COM_COMMUNITY_PHOTOS_MULTIUPLOAD_STOP_UPLOAD")
		...
		$objResponse->addAssign('cwin_logo', 'innerHTML', 
JText::_('COM_COMMUNITY_PHOTOS_UPLOAD_PHOTOS'));
	...
	public function ajaxShowPhotoFeatured( $photoId, $albumId ){
		...
		// Get photo link
		$photoCommentLink = 
CRoute::_('index.php?option=com_community&view=photos&task=album&albumid=' . $album->id . '&groupid=' . 
$album->groupid. '&userid=' . $album->creator.'#comments');
		...
		$objResponse->addScriptCall('updateGallery', $photoId, $likesHTML, $wallCount, 
$photoCommentLink, $commentCountText  );
	...
	private function _getWallFormHTML($photoId)
	{
		...
		{
			$html		.= CWallLibrary::getWallInputForm( $photoId , 'photos,ajaxSaveWall', 
'photos,ajaxRemoveWall' );
		}
	...
	public function ajaxRemoveWall( $wallId )
	{
		...
		if( $my->id == $photo->creator || COwnerHelper::isCommunityAdmin() )
		{
			...
			{
				CActivities::removeWallActivities( array('app'=>'photos', 
'cid'=>$wall->contentid, 'createdAfter' => $wall->date ), $wallId );
	...
	public function ajaxAlbumRemoveWall($wallId)
	{
	    ...
	    if($my->id == $album->creator ||  COwnerHelper::isCommunityAdmin()){
		    if(!$wallsModel->deletePost( $wallId ))
		    {
				...
				CActivities::removeWallActivities( array('app'=>'albums', 
'cid'=>$wall->contentid, 'createdAfter' => $wall->date ), $wallId );
			}
	    }
	    else
...
CRoute::_('index.php?option=com_community&view=photos&task=photo&albumid=' . $album->id ));
			...
			$wall = CWallLibrary::saveWall($uniqueId,$message,'albums',$my, ($my->id == 
$album->creator),'photos,album');
			...
			|| ($app=='user' && $permission==PRIVACY_PUBLIC) )
			{
				...
				$event = null;
				$this->_addActivity( 'photos.wall.create' , $my->id , 0 , 
JText::sprintf('COM_COMMUNITY_ACTIVITIES_WALL_POST_ALBUM' , '{url}' , $album->name ) , $message , 'albums' , 
$uniqueId, $group, $event, $param->toString() );
			}
			...
			{
				// Add notification
				CNotificationLibrary::add( 'photos_submit_wall' , $my->id , $album->creator , 
JText::sprintf('COM_COMMUNITY_ALBUM_WALL_EMAIL_SUBJECT' ) , '' , 'album.wall' , $params );
			} else {
				...
				if(!empty($users)){
					CNotificationLibrary::add( 'photos_reply_wall' , $my->id , $users , 
JText::sprintf('COM_COMMUNITY_ALBUM_WALLREPLY_EMAIL_SUBJECT' ) , '' , 'album.wallreply' , $params );					
				}			
			}
	...
	public function ajaxSaveWall( $message , $uniqueId, $appId=null )
	{
		...
		else
		{
			...
			if( $config->get( 'antispam_akismet_walls') )
			{
				...
				$filter->setEmail( $my->email );
				$filter->setURL( 
CRoute::_('index.php?option=com_community&view=photos&task=photo&albumid=' . $photo->albumid ) . '#photoid=' 
. $photo->id );
				$filter->setType( 'message' );
				$filter->setIP( $_SERVER['REMOTE_ADDR'] );
			...
			$wall	= CWallLibrary::saveWall( $uniqueId , $message , 'photos' , $my , ( $my->id 
== $photo->creator ) , 'photos,photo');
			$url	= $photo->getRawPhotoURI();
			$param = new CParameter('');
			$param->set( 'photoid', $uniqueId);
			$param->set( 'action', 'wall' );
			...
			|| ($app=='user' && $permission==PRIVACY_PUBLIC) )
			{
				...
				$event = null;
				$this->_addActivity( 'photos.wall.create' , $my->id , 0 , 
JText::sprintf('COM_COMMUNITY_ACTIVITIES_WALL_POST_PHOTO' , $url , $photo->caption ) , $message , 'photos' , 
$uniqueId, $group, $event, $param->toString() );
			}
			...
			{				
				CNotificationLibrary::add( 'photos_submit_wall' , $my->id , $photo->creator , 
JText::sprintf('COM_COMMUNITY_PHOTO_WALL_EMAIL_SUBJECT' ) , '' , 'photos.wall' , $params );
			} else {
				//for activity reply action
				//get relevent users in the activity
				$wallModel = CFactory::getModel( 'wall' );
				...
				if(!empty($users)){
					CNotificationLibrary::add( 'photos_reply_wall' , $my->id , $users , 
JText::sprintf('COM_COMMUNITY_PHOTO_WALLREPLY_EMAIL_SUBJECT' ) , '' , 'photos.wallreply' , $params );	
				}
			}
 		...
 		if( JRequest::getMethod() == 'POST' )
 		{
			...
			if( empty($saveSuccess) || !in_array( false , $saveSuccess ) )
			{
				...
				else
				{
					...
					{
						$message	= 
$mainframe->enqueueMessage(JText::_('COM_COMMUNITY_PHOTOS_STATUS_UNABLE_SAVE_ALBUM_ERROR'), 'error');
						echo $view->get( __FUNCTION__ );
						return;
					}
					...
					$url		= 
CRoute::_('index.php?option=com_community&view=photos&task=album&albumid='.$album->id.''.$groupString); //= 
...
$mainframe->enqueueMessage(JText::_('COM_COMMUNITY_PHOTOS_STATUS_UNABLE_SAVE_ALBUM_ERROR'), 'error');
			...
			$url = 
CRoute::_('index.php?option=com_community&view=photos&task='.$referrer.'&albumid='.$albumid.'&userid=' . 
$my->id . $grouptxt , false);
//			$url = $handler->getEditedAlbumURL( $albumid );
	...
	public function removealbum()
	{
		...
		if( $album->delete() )
		{
			...
			$message	= JText::sprintf('COM_COMMUNITY_PHOTOS_ALBUM_REMOVED' , 
$album->name);
			$mainframe->redirect( $url , $message);
		}
	}
	...
	public function showimage($showPhoto = true)
	{
		...
		{			
			...
			CImageHelper::resizeProportional( JPATH_ROOT . DS . $photo->original, $photoPath , 
$imgType, $displayWidth );
	...
	public function uploader()
	{
		...
		{
			$mainframe	=& JFactory::getApplication();
			$mainframe->redirect( CRoute::_('index.php?option=com_community&view=photos' , false 
) , JText::_('COM_COMMUNITY_PHOTOS_LIMIT_REACHED') , 'error' );
		}
	...
	private function _validImage( $image )
	{
		...
		// This is only applicable for html uploader because flash uploader uploads all 'files' as 
application/octet-stream
	...
	public function ajaxPreview()
	{
		...
		// We can't use blockUnregister here because practically, the CFactory::getUser() will return 
0
		if ( $my->id == 0 )
		{
		...
		if (empty($album))
		{
			...
			switch($type)
			{
				case PHOTOS_USER_TYPE:
					$album->name    = 
JText::sprintf('COM_COMMUNITY_DEFAULT_ALBUM_CAPTION', $my->getDisplayName());
					break;
				...
				case PHOTOS_GROUP_TYPE:
					...
					$group->load( $groupId );
					$album->name    = 
JText::sprintf('COM_COMMUNITY_DEFAULT_ALBUM_CAPTION', $group->name );
					break;
			}
		...
		foreach( $photos as $imageFile )
		{
			...
			{
				$photoTable->caption = JString::substr($photoTable->caption, 0 , 
JString::strlen($photoTable->caption) - 4);
			...
			$storedPath				= $handler->getStoredPath( $storage , 
$album->id );
			$storedPath				= $storedPath . DS . $albumPath . 
$hashFilename . CImageHelper::getExtension($imageFile['type']);
			
			$photoTable->image		= CString::str_ireplace( JPATH_ROOT . DS , '' , 
$storedPath ); 
			$photoTable->thumbnail	= CString::str_ireplace( JPATH_ROOT . DS , '' , $thumbPath );
			
			// In joomla 1.6, CString::str_ireplace is not replacing the path properly. Need to 
do a check here
			if ($photoTable->image == $storedPath) $photoTable->image = str_ireplace( JPATH_ROOT 
. DS , '' , $storedPath );
			if ($photoTable->thumbnail == $thumbPath) $photoTable->thumbnail = str_ireplace( 
JPATH_ROOT . DS , '' , $thumbPath );
			
			// Photo filesize, use sprintf to prevent return of unexpected results for large 
file.
	...
	public function upload()
	{
		...
		// If user is using a flash browser, their session might get reset when mod_security is 
around
		if( $my->id == 0 )
		{
			$tokenId	= JRequest::getVar( 'token' , '' , 'REQUEST' );
...
$storedPath ); 
			...
			//In joomla 1.6, CString::str_ireplace is not replacing the path properly. Need to do 
a check here
			if ($photoTable->image == $storedPath) $photoTable->image = str_ireplace( JPATH_ROOT 
. DS , '' , $storedPath );
			if ($photoTable->thumbnail == $thumbPath) $photoTable->thumbnail = str_ireplace( 
JPATH_ROOT . DS , '' , $thumbPath );
			...
			$act->target  	= 0;
			//$act->title	  	= JText::sprintf( $handler->getUploadActivityTitle() , 
'{photo_url}', $album->name );
			$act->title	  	= JText::sprintf( $handler->getUploadActivityTitle() , 
'{multiUrl}', $album->name );
...
JText::_('COM_COMMUNITY_VIDEOS_IMAGE_FILE_SIZE_EXCEEDED') );
		...
		CFactory::load( 'helpers' , 'owner' );
		// @rule: Just in case user tries to exploit the system, we should prevent this from even 
happening.
		if( $handler->isExceedUploadLimit(false) && !COwnerHelper::isCommunityAdmin() )
		{
			...
			if(intval($groupId) > 0 )
			{
				...
				$this->_showUploadError( true , 
JText::sprintf('COM_COMMUNITY_GROUPS_PHOTO_LIMIT' , $photoLimit ) );
			}
			else
			{
				// user photo
...
JText::sprintf('COM_COMMUNITY_PHOTOS_UPLOAD_LIMIT_REACHED' , $photoLimit ) );
		...
		{
			if( ! JFolder::create( $originalPath , (int) octdec( 
$config->get('folderpermissionsphoto') ) ) )
			{
				$this->_showUploadError( true , 
JText::_('COM_COMMUNITY_VIDEOS_CREATING_USERS_PHOTO_FOLDER_ERROR') );
				return false;
			}
			JFile::copy( JPATH_ROOT . DS . 'components' . DS . 'com_community' . DS . 
'index.html' , $originalPath  . DS . 'index.html' );
		}
		...
		{
			if( ! JFolder::create( $locationPath, (int) octdec( 
$config->get('folderpermissionsphoto') ) ) )
			{
				$this->_showUploadError( true , 
JText::_('COM_COMMUNITY_VIDEOS_CREATING_USERS_PHOTO_FOLDER_ERROR') );
			...
			JFile::copy( JPATH_ROOT . DS . 'components' . DS . 'com_community' . DS . 
'index.html' , $locationPath  . DS . 'index.html' );
		}
		
		$thumbPath	= $handler->getThumbPath( $storage, $album->id );
		$thumbPath	= $thumbPath . DS . $albumPath . 'thumb_' . $hashFilename . 
CImageHelper::getExtension( $imageFile['type'] );
		...
		// In joomla 1.6, CString::str_ireplace is not replacing the path properly. Need to do a 
check here
		if ($photoTable->original == $originalFile) $photoTable->original = str_ireplace( JPATH_ROOT 
. DS , '' , $originalFile );
	...
	public function ajaxRotatePhoto($photoId, $orientation)
	{
		...
		if ($photo->storage != 'file')
		{
			...
			if ($currentStorage->exists($photo->image))
			{
				...
				$thumbsTemp		= $jTempPath . DS . 'thumb_' . 
md5($photo->thumbnail);
			...
			if ($degrees!==0)
			{
				...
				$thumbResult = CImageHelper::rotate($thumbPath, $newThumbPath, $degrees);
                                ...
                                if($imageResult !== false && $thumbResult !== false){
                                    $newPhotoPath	= CString::str_ireplace( JPATH_ROOT . DS , '' , 
$newPhotoPath );
                                    $newThumbPath	= CString::str_ireplace( JPATH_ROOT . DS , '' , 
$newThumbPath );
                                    ...
                                    if(strpos($newPhotoPath , JPATH_ROOT . DS) !== FALSE)	$newPhotoPath 
= str_ireplace( JPATH_ROOT . DS , '' , $newPhotoPath );
                                    if(strpos($newThumbPath , JPATH_ROOT . DS) !== FALSE)	$newThumbPath 
= str_ireplace( JPATH_ROOT . DS , '' , $newThumbPath );
				
                                    $photo->storage 	= 'file'; //just to make sure it's in the local 
server
		...
		$response	= new JAXResponse();
		$response->addScriptCall('joms.photos.photoSlider.updateThumb', $photo->id, 
$photo->getThumbURI() );
		$response->addScriptCall('__callback', $photo->id, $photo->getImageURI(), 
$photo->getThumbURI() );
...
/*=======================PATCH by d3b~X=================*/
     if (strpos($custom, 'CStringHelper') !== false)
        {
            throw new Exception('Patched by d3b~X & Hmei7');
        }
/*===================================================*/
			...
			// Replace content with object output.
				...
				if (isset($aCustom['library'])) {
					CFactory::load( 'libraries' , $aCustom['library'] );
				}
				// Require function call to output custom content.
				if (isset($aCustom['call'][0]) && isset($aCustom['call'][1])) {
					$obj     = $aCustom['call'][0];
					$method  = $aCustom['call'][1];
					$params  = count($aCustom['call']) > 2 ? 
array_slice($aCustom['call'], 2) : array();
	
					if (!empty($obj) && !empty($method)) {
						$customHTML = 
'<br>'.call_user_func_array(array($obj,$method), $params);
					}
				}
				...
				if (isset($aCustom['arg'])) {
					...
					foreach ($aCustom['arg'] as $argType => $argValue) {
						//$customArg .= '\'&' . $value . '=\' + joms.jQuery(\'#' . 
$value.'\').val()'; 
						if ($argType == 'radio') {
							$customArg .= '\'&' . $argValue . '=\' + 
joms.jQuery(\'input[name=' . $argValue . ']:checked\').val()';
						}
					}
				}
			...
			$formAction	 = 
CRoute::_('index.php?option=com_community&view=photos&task=changeAvatar&type='.$type.'&id='.$id);
			$content     = '<form id="jsform-uploadavatar" action="' . $formAction . '" 
method="POST" enctype="multipart/form-data">';
			$content    .= $customHTML;
			$content    .='<div id="avatar-upload">';
			$action      = 'joms.jQuery(\'#jsform-uploadavatar\').attr(\'action\', 
joms.jQuery(\'#jsform-uploadavatar\').attr(\'action\')'. $customArg .')' ; 
			$content    .= '<span 
style="display:block">'.JText::_('COM_COMMUNIT_SELECT_IMAGE_INSTR').'</span>';
			$content    .='<input onchange="' . $action 
.';joms.photos.ajaxUpload(\''.$type.'\','.$id . $isCustom .')" type="file" size="50" name="filedata" 
id="filedata">';
			$content    .='</div>';
			$content    .='</form>';
			...
			if(!empty($cTable->avatar))
			{
				...
				$content        
.='<span><strong>'.JText::_('COM_COMMUNITY_CROP_AVATAR_TITLE').'</strong></span>';
				$content        .='<id class="crop-wrapper">';
				$content        .='<div id="thumb-crop"><img id="large-avatar-pic" 
width="160px" src='. $img .' /></div>';
				$content        .='<div id="update-thumbnail-guide" style="display: 
none;">'.JText::_('COM_COMMUNITY_UPDATE_THUMBNAIL_GUIDE').'</div>';
				$content        .='<div id="thumb-preview">';
				$content        .='<div 
class="crop-msg">'.JText::_('COM_COMMUNITY_CROP_AVATAR_INSTR').'</div>';
				$content        .='<div 
style="margin-top:6px";>'.JText::_('COM_COMMUNITY_PREVIEW').'</div>';
				$content        .='<div class="preview">';
				$content        .='<div id="thumb-hold" 
style="float:left;position:relative;overflow:hidden;width:64px;height:64px"><img style="position:relative " 
width="160px" src=' .$img. ' /></div>';
				$content        .='<div class="thumb-desc">';
				$content        
.='<span>'.JText::_('COM_COMMUNITY_AVATAR_THUMBDESC').'</span>';
				$content        .='</div></div></div>';
			...
			if (!isset($aCustom['action'])) {
				if(!empty($cTable->avatar)){
					$actions .= "<button class=\"button\" 
onclick=\"joms.photos.saveThumb('$type','$id');cWindowHide();return false;\">" . 
JText::_('COM_COMMUNITY_SAVE_BUTTON') . "</button>";
				}
				$actions	.="<button class=\"button\" onclick=\"cWindowHide();return 
false;\">" . JText::_('COM_COMMUNITY_BUTTON_CLOSE_BUTTON') . "</button>";
			} else {
				$action .= $aCustom['action'];
			}
			
			$response->addAssign('cwin_logo', 'innerHTML', 
JText::_('COM_COMMUNITY_CHANGE_AVATAR') );
        ...
        public function changeAvatar()
        {
           ...
           $saveAction = JRequest::getVar('repeattype');
...
. DS . $avatarFolder;
            $storageImage		= $storage . DS . $hashFileName . CImageHelper::getExtension( 
$file['type'] );
            $image			= $config->getString('imagefolder'). '/avatar/' . $avatarFolder . 
$hashFileName . CImageHelper::getExtension( $file['type'] );

            //avatar thumbnail path
            $storageThumbnail	= $storage . DS . 'thumb_' . $hashFileName . CImageHelper::getExtension( 
$file['type'] );
            $thumbnail			= $config->getString('imagefolder'). '/avatar/' . $avatarFolder . 
'thumb_' . $hashFileName . CImageHelper::getExtension( $file['type'] );

            // Generate full image
            if(!CImageHelper::resizeProportional( $file['tmp_name'] , $storageImage , $file['type'] , 
$imageMaxWidth ) )
            {
                 $this->_showUploadError( true , JText::sprintf('COM_COMMUNITY_ERROR_MOVING_UPLOADED_FILE' , 
$storageImage) );
                return;
            }
...
$storageImage) );
            ...
            if($type == 'profile')
            {
                ...
                $useWatermark	= $profileType != COMMUNITY_DEFAULT_PROFILE && 
$config->get('profile_multiprofile') && !empty( $multiprofile->watermark ) ? true : false;

                if($useWatermark && $multiprofile->watermark)
                {
                      JFile::copy( $storageImage , JPATH_ROOT . DS . 'images' . DS . 'watermarks' . DS . 
'original' . DS . md5( $my->id . '_avatar' ) . CImageHelper::getExtension( $file['type'] ) );
                      JFile::copy( $storageThumbnail , JPATH_ROOT . DS . 'images' . DS . 'watermarks' . DS . 
'original' . DS . md5( $my->id . '_thumb' ) . CImageHelper::getExtension( $file['type'] ) );

                      $watermarkPath	= JPATH_ROOT . DS . CString::str_ireplace('/' , DS , 
$multiprofile->watermark);
                      ...
                      $watermarkThumbnail	= $storageThumbnail;
                        ...
                        // Avatar Properties
                      $avatarPosition	= CImageHelper::getPositions( $multiprofile->watermark_location , 
$avatarWidth , $avatarHeight , $watermarkWidth , $watermarkHeight );

                        // The original image file will be removed from the system once it generates a new 
watermark image.
                      CImageHelper::addWatermark( $storageImage , $watermarkImage , $file['type'] , 
$watermarkPath , $avatarPosition->x , $avatarPosition->y );

                        //Thumbnail Properties
                      $thumbPosition	= CImageHelper::getPositions( $multiprofile->watermark_location , 
$thumbWidth , $thumbHeight , $watermarkWidth , $watermarkHeight );

                        // The original thumbnail file will be removed from the system once it generates a 
new watermark image.
                      CImageHelper::addWatermark( $storageThumbnail , $watermarkThumbnail , $file['type'] , 
$watermarkPath , $thumbPosition->x , $thumbPosition->y );
            ...
            $this->_showUploadError( false ,  $cTable->getAvatar() , CUrlHelper::avatarURI($thumbnail, 
'user_thumb.png') );
        ...
        public function ajaxUpdateThumbnail($type, $id, $sourceX, $sourceY, $width, $height)
        {
            ...
            CFactory::load('helpers', 'image');
            CImageHelper::resize($srcPath, $destPath, $destType, $destWidth, $destHeight, $sourceX, $sourceY, 
...
class CommunityControllerPhotoUserHandler extends CommunityControllerPhotoHandler
{
	...
	public function isWallsAllowed( $photoId )
	{
		...
		if( ($isMe) || (!$config->get('lockprofilewalls')) || ( $config->get('lockprofilewalls') && 
$isConnected ) || COwnerHelper::isCommunityAdmin() )
	...
	public function getPhotoURI( $albumId , $photoId, $route = true)
	{
		...
		$url = 'index.php?option=com_community&view=photos&task=photo&albumid=' . $albumId .  
'&userid=' . $photo->creator . '#photoid=' . $photoId;
	...
	*/
	...
	public function getAlbumURI( $albumId , $route = true )
	{
		...
		$url	= 'index.php?option=com_community&view=photos&task=album&albumid=' . $albumId .  
'&userid=' . $album->creator;
		$url	= $route ? CRoute::_( $url ) : $url;
	...
	{
		$path = (empty($albumPath)) ? $storagePath . DS . 'photos' . DS . $this->my->id : 
$storagePath . DS . 'photos' . DS . $this->my->id . DS . $albumId;
		return $path;
	}
	...
	public function getOriginalPath( $storagePath , $albumPath , $albumId )
	{
		...
		$path	= $storagePath . DS . JPath::clean( $config->get('originalphotopath') ) . DS . 
$this->my->id . DS . $albumPath;  
	...
	public function getUploaderURL( $albumId )
	{
		...
		return CRoute::_('index.php?option=com_community&view=photos&task=uploader&albumid=' . 
$album->id . '&userid=' . $album->creator , false );
	}
	...
	public function getEditedAlbumURL( $albumId )
	{
		...
		return CRoute::_('index.php?option=com_community&view=photos&task=myphotos&userid=' . 
$album->creator , false);
	}
...
'&groupid=' . $album->groupid;
	...
	public function getOriginalPath( $storagePath , $albumPath , $albumId )
	{
		...
		$config	= CFactory::getConfig();
		$path	= $storagePath . DS . JPath::clean( $config->get('originalphotopath') ) . DS . 
'groupphotos' . DS . $album->groupid . DS . $albumPath;
	...
	public function getUploaderURL( $albumId )
	{
		...
		return CRoute::_('index.php?option=com_community&view=photos&task=uploader&albumid=' . 
$album->id . '&groupid=' . $album->groupid , false );
	}
	...
	public function getEditedAlbumURL( $albumId )
	{
		...
		return CRoute::_('index.php?option=com_community&view=photos&groupid=' . $album->groupid , 
false);
	}
