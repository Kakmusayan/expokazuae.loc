...
joms.extend({
	...
	comments:{
		...
		edit: function(id,parentid){
			...
			joms.jQuery('div[data-commentid='+id+']').hide();
			var temp = document.createElement("div");
			var oldValue 	= joms.jQuery('div[data-commentid='+id+'] .cStream-Content span.comment').html();

			var w = joms.jQuery('#'+ parentid +' form').parent().width();

			//joms.jQuery('#wall-cmt-'+ id +' .wall-coc-form-action.add').removeAttr('disabled');
			joms.jQuery('li[data-streamid='+ parentid + '] [data-formblock]').show();
			joms.jQuery('li[data-streamid='+ parentid + '] [data-formblock] form').width(w);

			// Hide the "reply" block
			joms.jQuery('li[data-streamid='+ parentid + '] [data-replyblock]').hide();

			// @todo: should only autogrow once.
			var textarea = joms.jQuery('li[data-streamid='+ parentid+'] .cStream-Form').find('textarea');
			temp.innerHTML = oldValue;
			var sanitized = temp.textContent || temp.innerText;
			textarea.val(sanitized);

			joms.jQuery('li[data-streamid='+ parentid + '] button').attr('onClick','joms.comments.saveEdit('+id+','+parentid+');return false;');
			joms.jQuery('li[data-streamid='+ parentid + '] .cStream-FormCancel').attr('onClick','joms.comments.cancelEdit('+id+','+parentid+');return false;');
