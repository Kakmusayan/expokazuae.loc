...
* @version $Id: cb.core.php 1913 2012-11-05 23:53:50Z beat $
		...
		if ( ( $output == 'htmledit' ) && ( $reason != 'search' ) && defined( '_CB_VALIDATE_NEW' ) ) {
			if ( $field->params->get( 'fieldValidateInBrowser', 1 ) == 1 ) {
				...
				if ( $pregExp ) {
					...
					$fieldType			=	in_array( $field->type, array( 'textarea', 'editorta' ) ) ? 'textarea' : 'text';
					return $this->_fieldEditToHtml( $field, $user, $reason, 'input', $fieldType, $value, '', null, true, array( $this->getMetaClass( $field, array( $validationId . ':true' ) ) ) );
				}
			}
		}
	...
	function _displayDateToSql( $value ) {
		...
		if ( $value !== null ) {
			...
			if ( ! preg_match( '/[0-9]{4}-[01][0-9]-[0-3][0-9]/', $value ) ) {
				$value					=	$_CB_database->getNullDate();
			}
		}
		return $value;
	}
	...
	function validate( &$field, &$user, $columnName, &$value, &$postdata, $reason ) {
		...
		switch ( $columnName ) {
			case 'upload':
				...
				// Pre-checks maximum file size here to flag validation error before committing time (fixing bug #3547):
				$maxSize	=	$this->_getImageFieldParam( $field, 'avatarSize' );
				if ( filesize( $value['tmp_name'] ) > ( $maxSize * 1024 ) ) {
					$this->_setValidationError( $field, $user, $reason, sprintf( CBTxt::T("The file exceeds the maximum size of %s kilobytes"), $maxSize ) );
					return false;
				}
				break;
			case 'gallery':
