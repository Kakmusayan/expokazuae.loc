...
class cbgjAdmin extends cbPluginHandler {
	...
	private function showCategoryEdit( $id, $user, $plugin, $message = null ) {
		...
		$categories					=	cbgjData::listArray( cbgjData::getCategories( null, array( 'id', '!=', (int) $row->get( 'id' ) ) ) );
	...
	private function showGroups( $user, $plugin ) {
		...
		if ( $listCategories ) {
			...
			$input['batch_category']		=	moscomprofilerHTML::selectList( $listCategories, 'batch_category', null, 'value', 'text', null, 1, false, false );
			...
			array_unshift( $listGroups, moscomprofilerHTML::makeOption( '', CBTxt::T( '- Select Parent -' ) ) );

			$input['batch_parent']			=	moscomprofilerHTML::selectList( $listGroups, 'batch_parent', null, 'value', 'text', null, 1, false, false );
			...
			$input['parent']				=	$paging->getInputSelect( 'adminForm', 'parent', $listGroups, $parent );
		} else {
			$input['category']				=	'-';
	...
	private function showGroupEdit( $id, $user, $plugin, $message = null ) {
		...
		if ( $categories ) {
			$input['category']				=	moscomprofilerHTML::selectList( cbgjData::listArray( $categories ), 'category', null, 'value', 'text', (int) cbgjClass::getCleanParam( true, 'category', $row->get( 'category', $category->get( 'id' ) ) ), 1, false, false );

			if ( $category->get( 'id' ) ) {
				$listGroups					=	cbgjData::listArray( cbgjData::getGroups( null, array( array( 'category', '=', (int) $category->get( 'id' ) ), array( 'id', '!=', (int) $row->get( 'id' ) ) ) ) );
			} else {
				$listGroups					=	array();

				foreach ( $categories as $cat ) {
					$groups					=	cbgjData::listArray( cbgjData::getGroups( null, array( array( 'category', '=', (int) $cat->get( 'id' ) ), array( 'id', '!=', (int) $row->get( 'id' ) ) ) ) );
			...
			if ( $listGroups ) {
				...
				$input['parent']			=	moscomprofilerHTML::selectList( $listGroups, 'parent', null, 'value', 'text', (int) cbgjClass::getCleanParam( true, 'parent', $row->get( 'parent', 0 ) ), 1, false, false );
			} else {
				if ( $category->get( 'id' ) ) {
	...
	private function showUserEdit( $id, $user, $plugin, $message = null ) {
		...
		$input['user']				=	'<input type="text" id="user_id" name="user_id" value="' . (int) cbgjClass::getCleanParam( true, 'user_id', $row->get( 'user_id', $user->id ) ) . '" class="inputbox required digits" size="6" />';
