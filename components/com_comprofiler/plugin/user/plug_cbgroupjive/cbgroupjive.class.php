...
class cbgjClass {
    ...
    static public function getPluginURL( $variables = array(), $msg = null, $htmlspecialchars = true, $redirect = false, $type = null, $return = false, $ajax = false, $back = false ) {
		...
		if ( $msg ) {
			if ( $redirect ) {
				cbgjClass::setRedirect( $url, $msg, $type );
			} else {
				if ( $msg === true ) {
    ...
    static public function getTemplate( $files = null ) {
		...
		$id									=	cbgjClass::getStaticID( $files );
		...
		if ( ! isset( $tmpl[$id] ) ) {
			...
			foreach ( $files as $file ) {
				...
				$_PLUGINS->trigger( 'gj_onBeforeTemplate', array( $file, $template, $plugin ) );
				...
				$_PLUGINS->trigger( 'gj_onAfterTemplate', array( $file, $template, array( $globalCss, $php, $css, $js, $overrideCss ), $plugin ) );
    ...
    static public function getFormValidation( $selector = '#gjForm', $params = null ) {
		...
		if ( ! isset( $vld[$selector] ) ) {
			...
			if ( $params ) {
				$js			.=		$params . ( ( substr( trim( $params ), -1 ) != ',' ) ? ',' : null );
			}
    ...
    static public function getIcon( $msg, $title = null, $class = 'icon-question-sign', $forceTooltip = false ) {
		...
		if ( $msg ) {
			if ( ( $generalTooltips == 1 ) || $forceTooltip ) {
				...
				return '<span class="gjPopOver"><i class="' . htmlspecialchars( $class ) . '"></i>' . $popover . '</span>&nbsp;';
			...
			return '<span class="gjTooltip"><i class="' . htmlspecialchars( $class ) . '"></i>' . $tooltip . '</span>&nbsp;';
		}
...
class cbgjCategory extends comprofilerDBTable {
	...
	public function getName( $length = 0, $linked = false ) {
		...
		if ( $name ) {
			...
			if ( $length && ( strlen( $name ) > $length ) ) {
				$name	=	rtrim( trim( substr( $name, 0, $length ) ), '.' ) . '...';
				$short	=	true;
			} else {
				$short	=	false;
			}
	...
	public function getDescription( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$cache[$id]			=	( $plugin->params->get( 'category_editor', 1 ) >= 2 ? $this->get( 'description' ) : htmlspecialchars( $this->get( 'description' ) ) );
		}
		...
		if ( $description ) {
			...
			if ( $length && ( strlen( strip_tags( $description ) ) > $length ) ) {
				$description	=	rtrim( trim( substr( strip_tags( $description ), 0, $length ) ), '.' ) . '...';
			}
		}
	...
	public function getGroups() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getGroups( null, array( 'category', '=', (int) $id ) );
		}
	...
	public function getNotifications() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getNotifications( null, array( array( 'type', '=', 'category' ), array( 'item', '=', (int) $id ) ) );
		}
	...
	public function getNested() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getCategories( null, array( 'parent', '=', (int) $id ) );
		}
	...
	public function groupCount() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	count( cbgjData::getGroups( array( 'grp_access', 'mod_lvl2' ), array( 'category', '=', (int) $id ) ) );
		}
	...
	public function nestedCount() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$count			=	0;
			$categories		=	cbgjData::getCategories( array( 'cat_access', 'mod_lvl1' ), array( 'parent', '>', 0 ) );
...
class cbgjGroup extends comprofilerDBTable {
	...
	public function getDescription( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$cache[$id]			=	( $plugin->params->get( 'group_editor', 1 ) >= 2 ? $this->get( 'description' ) : htmlspecialchars( $this->get( 'description' ) ) );
		}
	...
	public function getUsers() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getUsers( null, array( 'group', '=', (int) $id ) );
		}
	...
	public function getInvites() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getInvites( null, array( 'group', '=', (int) $id ) );
		}
	...
	public function getNested() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getGroups( null, array( 'parent', '=', (int) $id ) );
		}
	...
	public function getNotifications() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getNotifications( null, array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $id ) ) );
		}
	...
	public function nestedCount() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$count			=	0;
			$groups			=	cbgjData::getGroups( array( 'grp_access', 'mod_lvl2' ), array( array( 'parent', '>', 0 ), array( 'category', '=', (int) $this->get( 'category' ) ) ) );
...
class cbgjUser extends comprofilerDBTable {
    ...
    public function getInvites() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getInvites( null, array( array( 'id', '=', (int) $this->get( 'group' ) ), array( 'user_id', '=', (int) $this->get( 'user_id' ) ) ) );
		}
	...
	public function getInvited() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getInvites( null, array( array( 'id', '=', (int) $this->get( 'group' ) ), array( 'user', '=', (int) $this->get( 'user_id' ) ) ) );
		}
	...
	public function getAllInvites() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getInvites( null, array( array( 'group', '=', (int) $this->get( 'group' ) ), array( 'user_id', '=', (int) $this->get( 'user_id' ), array( 'user', '=', (int) $this->get( 'user_id' ) ) ) ) );
		}
	...
	public function getNotifications() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	cbgjData::getNotifications( null, array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $this->get( 'group' ) ), array( 'user_id', '=', (int) $this->get( 'user_id' ) ) ) );
		}
