			...
			foreach ( $rows as $row ) {
				...
				if ( $beforeMenu || $ban || $delete || $promote || $demote || $afterMenu ) {
					$return				.=						'<div class="gjDropdown">'
										...
										.								$beforeMenu . $ban . $delete . $promote . $demote . $afterMenu
										.							'</div>'
										.						'</div>';
				}

				$return					.=						cbgjClass::getIntegrations( 'gj_onAfterGroupUserInfo', array( $row, $group, $category, $user, $plugin ) )
										.					'</div>';

				if ( ( $row->get( 'status' ) == 0 ) && cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
					$return				.=					'<div class="gjContentBoxRow">'
										.						'<input type="button" value="' . htmlspecialchars( CBTxt::Th( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . $activeUrl . '" />'
										.					'</div>';
				}
