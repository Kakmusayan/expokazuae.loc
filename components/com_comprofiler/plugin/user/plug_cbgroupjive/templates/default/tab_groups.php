...
class HTML_groupjiveTabGroups {

	/**
	 * render frontend tab groups
	 *
	 * @param object $rows
	 ...
	 * @return mixed
	 */
	static function showGroups( $rows, $pageNav, $displayed, $user, $plugin, $tabbed ) {
		global $_CB_framework;

		$groupTabSearch		=	$plugin->params->get( 'group_tab_search', 1 );
		$groupTabPaging		=	$plugin->params->get( 'group_tab_paging', 1 );
		$groupTabLimitbox	=	$plugin->params->get( 'group_tab_limitbox', 1 );

		if ( ! $tabbed ) {
			$formUrl		=	cbgjClass::getPluginURL( array( 'panel', 'groups' ) );
		} else {
			$formUrl		=	$_CB_framework->userProfileUrl( $displayed->id, true, $plugin->tab->tabid );
		}

		$return				=	'<div class="gjTabGroups">'
							.		'<form action="' . $formUrl . '" method="post" name="gjTabForm_groups" id="gjTabForm_groups" class="gjForm">'
							.			( $groupTabSearch && ( $pageNav->searching || $pageNav->total ) ? '<div class="gjTop gjTopRight">' . $pageNav->search . '</div>' : null );
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( $groupTabSearch && $pageNav->searching ) {
				$return		.=				CBTxt::Ph( 'No [group] search results found.', array( '[group]' => cbgjClass::getOverride( 'group' ) ) );
			} else {
				if ( $displayed->id == $user->id ) {
					$return	.=				CBTxt::Ph( 'You have no [groups].', array( '[groups]' => cbgjClass::getOverride( 'group', true ) ) );
				} else {
					$return	.=				CBTxt::Ph( 'This user has no [groups].', array( '[groups]' => cbgjClass::getOverride( 'group', true ) ) );
				}
			}
		...
		if ( $groupTabPaging ) {
			$return			.=			'<div class="gjPaging pagination pagination-centered">'
							...
							.				( ! $groupTabLimitbox ? $pageNav->getLimitBox( false ) : ( $pageNav->total ? '<div class="gjPagingLimitbox">' . $pageNav->limitbox . '</div>' : null ) )
							.			'</div>';
		}
