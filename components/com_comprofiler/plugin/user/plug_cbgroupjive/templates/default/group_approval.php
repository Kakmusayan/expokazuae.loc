	...
	static function showGroupApproval( $rows, $pageNav, $user, $plugin ) {
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( cbgjClass::hasAccess( 'grp_can_publish', $authorized ) ) {
				$return				.=							'<input type="button" value="' . htmlspecialchars( CBTxt::T( 'Approve' ) ) . '" class="gjButton gjButtonCancel btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'groups', 'publish', (int) $category->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />';
			}
