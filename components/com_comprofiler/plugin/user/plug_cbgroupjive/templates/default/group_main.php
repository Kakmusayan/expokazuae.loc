		...
		if ( ( ! $tabContent ) && ( ! $beforeTab ) && ( ! $afterTab ) ) {
			if ( $user->id ) {
				if ( ( $row->get( 'type' ) == 2 ) && cbgjClass::hasAccess( 'grp_usr_inactive', $authorized ) ) {
					$return		.=		'<div>'
								.			CBTxt::Ph( 'Please await approval to join the [group] to view its content.', array( '[group]' => cbgjClass::getOverride( 'group' ) ) )
								.		'</div>';
				} elseif ( ! cbgjClass::hasAccess( 'grp_approved', $authorized ) ) {
					if ( $plugin->params->get( 'group_approve', 0 ) ) {
						$return	.=		'<div>'
								.			CBTxt::Ph( 'Please await [group] approval.', array( '[group]' => cbgjClass::getOverride( 'group' ) ) )
								.		'</div>';
					} else {
						$return	.=		'<div>'
								.			CBTxt::Ph( '[group] is not yet published.', array( '[group]' => cbgjClass::getOverride( 'group' ) ) )
								.		'</div>';
					}
				} else {
					$return		.=		'<div>'
