...
class HTML_groupjiveInvites {
	...
	static function showInvites( $rows, $pageNav, $category, $group, $user, $plugin ) {
		...
		$return						=	'<form action="' . cbgjClass::getPluginURL( array( 'invites', 'send', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ) ) . '" method="post" name="gjForm_invite" id="gjForm_invite" class="gjForm gjToggle form-horizontal">'
									...
									.				'<input type="text" size="35" class="inputbox required" value="' . htmlspecialchars( $data->get( 'invites_invite', null ) ) . '" name="invites_invite" id="invites_invite" />'
		...
		if ( $rows ) {
			...
			foreach ( $rows as $row ) {
				...
				$return				.=			'<div class="gjContentBox mini-layout">'
									...
									.					'<div class="gjDropdown">'
									.						'<i class="icon-chevron-down"></i>'
									.						'<div class="gjDropdownItems">'
									.							cbgjClass::getIntegrations( 'gj_onBeforeGroupInviteMenu', array( $row, $group, $category, $user, $plugin ) )
				...
				if ( ( ! $row->isAccepted() ) && ( $row->dateDifference() >= 5 ) ) {
					$return			.=				'<div class="gjContentBoxRow">'
									.					'<input type="button" value="' . htmlspecialchars( CBTxt::Th( 'Resend' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'invites', 'send', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true ) . '" />'
									.				'</div>';
				}
