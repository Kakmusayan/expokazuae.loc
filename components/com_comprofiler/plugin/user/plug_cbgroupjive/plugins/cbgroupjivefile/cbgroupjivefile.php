...
class cbgjFilePlugin extends cbPluginHandler {
	...
	public function getFileApproval( $user, $plugin ) {
		...
		$where[]			=	array( 'published', '=', 0, 'c.params', 'CONTAINS', 'file_approve=1' );
			...
			foreach ( $rows as $row ) {
				...
				if ( $row->get( 'file' ) || cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized  ) ) {
					$return			.=						'<div class="gjDropdown">'
									...
									.								( $row->get( 'file' ) ? '<div><a href="' . $row->getDownloadUrl() . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Download' ) . '</a></div>' : null )
				...
				if ( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
					$return			.=					'<div class="gjContentBoxRow">'
									.						'<input type="button" value="' . htmlspecialchars( CBTxt::Th( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'file_publish', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />'
									.					'</div>';
				}
	...
	private function saveFile( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || cbgjClass::hasAccess( 'file_upload', $authorized ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'mod_lvl4', $authorized ), 'published', $row->get( 'published', ( $fileApprove ? 0 : 1 ) ) ) );
			$row->set( 'user_id', (int) $row->get( 'user_id', $user->id ) );
			$row->set( 'group', (int) $row->get( 'group', $group->get( 'id' ) ) );
			$row->set( 'title', cbgjClass::getCleanParam( true, 'file_title', $row->get( 'title' ) ) );
			$row->set( 'description', cbgjClass::getCleanParam( true, 'file_description', $row->get( 'description' ) ) );
			...
			$notifications					=	cbgjData::getNotifications( array( array( 'file_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', $group->id ), array( 'user_id', '!IN', array( $user->id, $row->user_id ) ), array( 'params', 'CONTAINS', 'group_filenew=1', array( 'params', 'CONTAINS', 'group_fileupdate=1' ), array( 'params', 'CONTAINS', 'group_fileapprove=1' ) ) ) );
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'group_fileupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $fileApprove && ( ! cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) && $notificationParams->get( 'group_fileapprove' ) ) {
						$message			=	CBTxt::P( '[user] uploaded [file_title_linked] in [group] and requires approval!', $row->getSubstitutionExtras( true ) );
					} elseif ( $notificationParams->get( 'group_filenew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::T( 'File saved successfully!' ), false, true, null, false, false, true );
		} else {
			if ( $group->get( 'id' ) ) {
				$url						=	array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) );
			} elseif ( $category->get( 'id' ) ) {
...
class cbgjFile extends comprofilerDBTable {
	...
	public function getFileName() {
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	( $this->get( 'file' ) ? substr( rtrim( pathinfo( $this->getCleanFile(), PATHINFO_BASENAME ), '.' . $this->getFileExt() ), 0, -14 ) . '.' . $this->getFileExt() : null );
		}
