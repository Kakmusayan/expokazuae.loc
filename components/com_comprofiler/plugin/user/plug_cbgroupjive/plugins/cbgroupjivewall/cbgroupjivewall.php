...
class cbgjWallPlugin extends cbPluginHandler {
	...
	public function getWallApproval( $user, $plugin ) {
		...
		$where[]			=	array( 'published', '=', 0, 'c.params', 'CONTAINS', 'wall_approve=1' );
	...
	private function showWallApproval( $rows, $pageNav, $user, $plugin ) {
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
				$return			.=							'<input type="button" value="' . htmlspecialchars( CBTxt::T( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'wall_publish', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />';
			}
			...
			if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) ) {
				$return			.=							'<div class="gjDropdown">'
								...
								.									( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'wall_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), null, true, false, null, true ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
	...
	public function getWall( $tabs, $group, $category, $user, $plugin ) {
		...
		cbgjClass::getTemplate( 'cbgroupjivewall' );
	...
	private function showWall( $rows, $pageNav, $tabs, $group, $category, $user, $plugin ) {
		...
		if ( cbgjClass::hasAccess( 'wall_post', $authorized ) ) {
			...
			if ( $inputLimit && ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) ) ) {
				$inputJs			=	"$( '#wall_post' ).bind( 'keypress keyup focus blur', function() {"
									.		"var length = $( this ).val().length;"
									.		"var count = ( " . (int) $inputLimit . " - length );"
									.		"$( '#wall_inputlimit' ).attr( 'value', ( count > 0 ? count : 0 ) );"
									.		"if ( count <= 0 ) {"
									.			"var value = $( this ).val();"
									.			"var new_value = value.substr( 0, " . (int) $inputLimit . " );"
									.			"$( this ).attr( 'value', new_value );"
									.		"}"
									.	"});"
									.	"$( '#wall_post_parent' ).live( 'click mouseenter mouseleave', function() {"
									.		"var editor = $( '#wall_post_parent iframe' ).contents().find( 'body' );"
			...
			if ( $wallEditor == 3 ) {
				...
				$input['post']		=	'<textarea id="wall_post" name="wall_post" class="input-xlarge" cols="40" rows="6">' . htmlspecialchars( $post ) . '</textarea>';
			}
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( $wallReply && cbgjClass::hasAccess( 'wall_post', $authorized ) ) {
				$return				.=							'<a href="javascript: void(0);" role="button" class="gjButton btn gjWallReply" rel="' . (int) $row->get( 'id' ) . '">' . CBTxt::Th( 'Reply' ) . '</a>';
			}
	...
	private function editWall( $catid, $grpid, $id, $user, $plugin, $message = null ) {
		...
		if ( $row->get( 'id' ) ) {
			...
			cbgjClass::getTemplate( 'cbgroupjivewall_edit' );
			...
			if ( $inputLimit && ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) ) ) {
				$inputJs			=	"$( '#wall_post' ).bind( 'keypress keyup focus blur', function() {"
									.		"var length = $( '#wall_post' ).val().length;"
									.		"var count = ( " . (int) $inputLimit . " - length );"
									.		"$( '#wall_inputlimit' ).attr( 'value', ( count > 0 ? count : 0 ) );"
									.		"if ( count <= 0 ) {"
									.			"var value = $( '#wall_post' ).val();"
									.			"var new_value = value.substr( 0, " . (int) $inputLimit . " );"
									.			"$( '#wall_post' ).attr( 'value', new_value );"
									.		"}"
									.	"});"
									.	"$( '#wall_post_parent' ).live( 'click mouseenter mouseleave', function() {"
									.		"var editor = $( '#wall_post_parent iframe' ).contents().find( 'body' );"
			...
			if ( class_exists( 'HTML_cbgroupjivewallEdit' ) ) {
				...
				if ( $inputLimit && ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) ) ) {
					$charCount		=	( $inputLimit - strlen( $row->get( 'post' ) ) );
	...
	private function saveWall( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || cbgjClass::hasAccess( 'wall_post', $authorized ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'mod_lvl4', $authorized ), 'published', $row->get( 'published', ( $wallApprove ? 0 : 1 ) ) ) );
			...
			if ( $plugin->params->get( 'wall_editor', 1 ) >= 2 ) {
				$row->set( 'post', cbgjClass::getHTMLCleanParam( true, 'wall_post', $row->get( 'post' ) ) );
			} else {
				$row->set( 'post', cbgjClass::getCleanParam( true, 'wall_post', $row->get( 'post' ) ) );
			}

			if ( $inputLimit && ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) ) && ( strlen( $row->get( 'post' ) ) > $inputLimit ) ) {
				$row->set( 'post', trim( substr( $row->get( 'post' ), 0, ( $inputLimit - 3 ) ) ) . '...' );
			}
			...
			if ( $row->get( 'reply' ) && $new ) {
				$notification			=	cbgjData::getNotifications( array( array( 'wall_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $group->get( 'id' ) ), array( 'user_id', '=', (int) $row->getReply()->get( 'user_id' ) ), array( 'params', 'CONTAINS', 'group_wallreply=1', 'params', '!CONTAINS', 'group_wallapprove=1' ) ), null, null, false );
			...
			$notifications				=	cbgjData::getNotifications( array( array( 'wall_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $group->get( 'id' ) ), array( 'user_id', '!IN', ( $replyNotify ? array( (int) $user->id, (int) $row->get( 'user_id' ), (int) $row->getReply()->get( 'user_id' ) ) : array( (int) $user->id, (int) $row->get( 'user_id' ) ) ) ), array( 'params', 'CONTAINS', 'group_wallnew=1', array( 'params', 'CONTAINS', 'group_wallupdate=1' ), array( 'params', 'CONTAINS', 'group_wallapprove=1' ) ) ) );
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'group_wallupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $wallApprove && ( ! cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) && $notificationParams->get( 'group_wallapprove' ) ) {
						$message		=	CBTxt::P( '[user] created [wall_title_linked] in [group] and requires approval!', $row->getSubstitutionExtras( true ) );
					} elseif ( $notificationParams->get( 'group_wallnew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::T( 'Wall saved successfully!' ), false, true, null, false, false, true );
		} else {
			if ( $group->get( 'id' ) ) {
				$url					=	array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) );
			} elseif ( $category->get( 'id' ) ) {
			...
			if ( class_exists( 'HTML_cbgroupjivewallParams' ) ) {
				...
				if ( $wallPost || cbgjClass::hasAccess( 'usr_mod', $authorized ) ) {
					$return		.=		'<div class="gjEditContentInput control-group">'
								...
								.				$input['wall_post']
								.				'<span class="gjEditContentInputIcon help-inline">'
								.					cbgjClass::getIcon( CBTxt::P( 'Select [group] post access. Post access determines what type of [users] can post to your %1$s wall (e.g. [users] signify only those a member of your [group] can post). The [users] above the selected will also have access.', array( '[group]' => cbgjClass::getOverride( 'group' ), '[users]' => cbgjClass::getOverride( 'user', true ) ) ) )
								.				'</span>'
								.			'</div>'
								.		'</div>';
				}
...
class cbgjWall extends comprofilerDBTable {
	...
	public function getPost( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$cache[$id]	=	( $plugin->params->get( 'wall_editor', 1 ) >= 2 ? $this->get( 'post' ) : htmlspecialchars( $this->get( 'post' ) ) );
		}
		...
		if ( $post ) {
			...
			if ( $length && ( strlen( strip_tags( $post ) ) > $length ) ) {
				$post	=	rtrim( trim( substr( strip_tags( $post ), 0, $length ) ), '.' ) . '...';
			}
		}
