...
class cbgjEventsPlugin extends cbPluginHandler {
	...
	private function getEventApproval( $user, $plugin ) {
		...
		$where[]			=	array( 'published', '=', 0, 'c.params', 'CONTAINS', 'events_approve=1' );
	...
	private function showEventsApproval( $rows, $pageNav, $user, $plugin ) {
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
				$return					.=							'<input type="button" value="' . htmlspecialchars( CBTxt::T( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'events_publish', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />';
			}
			...
			if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) || $canMessage || $canAttend || $canMaybeAttend || $canNotAttend ) {
				$return					.=							'<div class="gjDropdown">'
										...
										.									( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'events_edit', $category->id, $group->id, $row->id ), null, true, false, null, true ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
	...
	private function showEvents( $rows, $pageNav, $tabs, $group, $category, $user, $plugin ) {
		...
		if ( cbgjClass::hasAccess( 'events_schedule', $authorized ) ) {
			...
			if ( $eventsEditor == 3 ) {
				...
				$input['event']			=	'<textarea id="events_event" name="events_event" class="input-xlarge" cols="40" rows="6">' . htmlspecialchars( $event ) . '</textarea>';
			}

			$input['location']			=	'<input type="text" size="35" class="input-large" value="' . htmlspecialchars( cbgjClass::getCleanParam( true, 'events_location' ) ) . '" name="events_location" id="events_location" />';
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( $canAttend && $canMaybeAttend && $canNotAttend ) {
				$unsureAttend			=	true;

				$return					.=							'<input type="button" value="' . htmlspecialchars( CBTxt::T( 'Attend' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'events_yes', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), CBTxt::T( 'Are you sure you want to attend this event?' ) ) . '" />';
			} else {
				$unsureAttend			=	false;
			}
			...
			if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) || $canMessage || $canAttend || $canMaybeAttend || $canNotAttend ) {
				$return					.=							'<div class="gjDropdown">'
										.								'<i class="icon-chevron-down"></i>'
										.								'<div class="gjDropdownItems">'
										.									( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'events_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ) ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
										.									( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ? $state : null )
	...
	private function editEvent( $catid, $grpid, $id, $user, $plugin, $message = null ) {
		...
		if ( $row->get( 'id' ) ) {
			...
			$input['location']	=	'<input type="text" size="35" class="input-large" value="' . htmlspecialchars( cbgjClass::getCleanParam( true, 'events_location', $row->get( 'location' ) ) ) . '" name="events_location" id="events_location" />';
	...
	private function saveEvent( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || cbgjClass::hasAccess( 'events_schedule', $authorized ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'mod_lvl4', $authorized ), 'published', $row->get( 'published', ( $eventsApprove ? 0 : 1 ) ) ) );
			$row->set( 'user_id', (int) $row->get( 'user_id', $user->id ) );
			$row->set( 'group', (int) $row->get( 'group', $group->get( 'id' ) ) );
			$row->set( 'title', cbgjClass::getCleanParam( true, 'events_title', $row->get( 'title' ) ) );

			if ( $plugin->params->get( 'events_editor', 1 ) >= 2 ) {
				$row->set( 'event', cbgjClass::getHTMLCleanParam( true, 'events_event', $row->get( 'event' ) ) );
			} else {
				$row->set( 'event', cbgjClass::getCleanParam( true, 'events_event', $row->get( 'event' ) ) );
			}

			$row->set( 'location', cbgjClass::getCleanParam( true, 'events_location', $row->get( 'location' ) ) );
			$row->set( 'address', cbgjClass::getCleanParam( $plugin->params->get( 'events_plotting', 1 ), 'events_address', $row->get( 'address' ) ) );
			...
			$notifications					=	cbgjData::getNotifications( array( array( 'events_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', $group->id ), array( 'user_id', '!IN', array( $user->id, $row->user_id ) ), array( 'params', 'CONTAINS', 'group_eventnew=1', array( 'params', 'CONTAINS', 'group_eventupdate=1' ), array( 'params', 'CONTAINS', 'group_eventapprove=1' ) ) ) );
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'group_eventupdate' ) ) {
						$subject			=	CBTxt::P( '[group_name] - Event Edited!', $row->getSubstitutionExtras( true ) );
						$message			=	CBTxt::P( '[user] edited [event_url] in [group]!', $row->getSubstitutionExtras( true ) );
					}
				} else {
					$subject				=	CBTxt::P( '[group_name] - Event Scheduled!', $row->getSubstitutionExtras( true ) );

					if ( ( $row->get( 'published' ) == 0 ) && $eventsApprove && ( ! cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) && $notificationParams->get( 'group_eventapprove' ) ) {
						$message			=	CBTxt::P( '[user] scheduled [event_url] in [group] and requires approval!', $row->getSubstitutionExtras( true ) );
					} elseif ( $notificationParams->get( 'group_eventnew' ) ) {
						$message			=	CBTxt::P( '[user] scheduled [event_url] in [group]!', $row->getSubstitutionExtras( true ) );
					}
				}
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::T( 'Event saved successfully!' ), false, true, null, false, false, true );
		} else {
			if ( $group->get( 'id' ) ) {
				$url						=	array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) );
			} elseif ( $category->get( 'id' ) ) {
...
class cbgjEvent extends comprofilerDBTable {
	...
	public function getTitle( $length = 0, $linked = false ) {
		...
		if ( $title ) {
			...
			if ( $length && ( strlen( $title ) > $length ) ) {
				$title	=	rtrim( trim( substr( $title, 0, $length ) ), '.' ) . '...';
				$short	=	true;
			} else {
				$short	=	false;
			}
	...
	public function getEvent( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			...
			$cache[$id]	=	( $plugin->params->get( 'events_editor', 1 ) >= 2 ? $this->get( 'event' ) : htmlspecialchars( $this->get( 'event' ) ) );
		}
		...
		if ( $event ) {
			...
			if ( $length && ( strlen( strip_tags( $event ) ) > $length ) ) {
				$event	=	rtrim( trim( substr( strip_tags( $event ), 0, $length ) ), '.' ) . '...';
			}
		}
