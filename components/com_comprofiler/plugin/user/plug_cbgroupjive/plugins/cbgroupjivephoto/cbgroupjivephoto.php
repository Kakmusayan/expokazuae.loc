...
class cbgjPhotoPlugin extends cbPluginHandler {
	...
	public function getPhotoApproval( $user, $plugin ) {
		...
		$where[]			=	array( 'published', '=', 0, 'c.params', 'CONTAINS', 'photo_approve=1' );
			...
			foreach ( $rows as $row ) {
				...
				if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) ) {
					$return			.=						'<div class="gjDropdown">'
									...
									.								( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'photo_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), null, true, false, null, true ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
				...
				if ( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
					$return			.=					'<div class="gjContentBoxRow">'
									.						'<input type="button" value="' . htmlspecialchars( CBTxt::Th( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'photo_publish', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />'
									.					'</div>';
				}
			...
			foreach ( $rows as $row ) {
				...
				if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) ) {
					$return			.=						'<div class="gjDropdown">'
									.							'<i class="icon-chevron-down"></i>'
									.							'<div class="gjDropdownItems">'
									.								( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'photo_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ) ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
	...
	private function savePhoto( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || cbgjClass::hasAccess( 'photo_upload', $authorized ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'mod_lvl4', $authorized ), 'published', $row->get( 'published', ( $photoApprove ? 0 : 1 ) ) ) );
			$row->set( 'user_id', (int) $row->get( 'user_id', $user->id ) );
			$row->set( 'group', (int) $row->get( 'group', $group->get( 'id' ) ) );
			$row->set( 'title', cbgjClass::getCleanParam( true, 'photo_title', $row->get( 'title' ) ) );
			$row->set( 'description', cbgjClass::getCleanParam( true, 'photo_description', $row->get( 'description' ) ) );
			...
			$notifications					=	cbgjData::getNotifications( array( array( 'photo_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $group->get( 'id' ) ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'group_photonew=1', array( 'params', 'CONTAINS', 'group_photoupdate=1' ), array( 'params', 'CONTAINS', 'group_photoapprove=1' ) ) ) );
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'group_photoupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $photoApprove && ( ! cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) && $notificationParams->get( 'group_photoapprove' ) ) {
						$message			=	CBTxt::P( '[user] uploaded [photo_title_linked] in [group] and requires approval!', $row->getSubstitutionExtras( true ) );
					} elseif ( $notificationParams->get( 'group_photonew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::T( 'Photo saved successfully!' ), false, true, null, false, false, true );
		} else {
			if ( $group->get( 'id' ) ) {
				$url						=	array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) );
			} elseif ( $category->get( 'id' ) ) {
