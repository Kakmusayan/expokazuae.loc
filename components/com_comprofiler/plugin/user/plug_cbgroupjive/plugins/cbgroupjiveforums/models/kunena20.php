...
class cbgjForumsModel extends cbPluginHandler {
	...
	static public function setForum( $parentForum, $row, $user, $plugin ) {
		...
		if ( $parentForum ) {
			...
			$category->set( 'name', $row->getName() );
			$category->set( 'alias', KunenaRoute::stringURLSafe( ( $row->get( 'id' ) . ' ' . $row->getName() ) ) );
			$category->set( 'accesstype', 'communitybuilder' );
			$category->set( 'access', 0 );
			$category->set( 'pub_access', 1 );
			$category->set( 'pub_recurse', 1 );
			$category->set( 'admin_access', 0 );
			$category->set( 'admin_recurse', 1 );
