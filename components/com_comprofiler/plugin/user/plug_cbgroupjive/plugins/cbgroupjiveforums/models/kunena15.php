...
class cbgjForumsModel extends cbPluginHandler {
	...
	static public function getForum( $tabs, $row, $user, $plugin ) {
		...
		$query				=	'SELECT COUNT(*)'
							.	"\n FROM " . $_CB_database->NameQuote( '#__fb_messages' ) . " AS a"
							.	"\n LEFT JOIN " . $_CB_database->NameQuote( '#__fb_messages_text' ) . " AS b"
							.	' ON a.' . $_CB_database->NameQuote( 'id' ) . ' = b.' . $_CB_database->NameQuote( 'mesid' )
							.	"\n LEFT JOIN " . $_CB_database->NameQuote( '#__fb_categories' ) . " AS c"
							.	' ON a.' . $_CB_database->NameQuote( 'catid' ) . ' = c.' . $_CB_database->NameQuote( 'id' )
							.	"\n LEFT JOIN " . $_CB_database->NameQuote( '#__fb_messages' ) . " AS d"
	...
	static public function setForum( $parentForum, $row, $user, $plugin ) {
		...
		if ( $parentForum ) {
			...
			$query				=	'INSERT INTO ' . $_CB_database->NameQuote( '#__fb_categories' )
								.	"\n ("
								...
								.	"\n VALUES ("
								.		(int) $parentForum
								.		', ' . $_CB_database->Quote( $row->getName() )
								.		', 1'
								.		', 0'
								.		', 0'
								.		', 0'
	...
	static public function setModerator( $userId, $row ) {
		...
		$query			=	'SELECT *'
						.	"\n FROM " . $_CB_database->NameQuote( '#__fb_moderation' )
		...
		if ( ! $moderator ) {
			...
			$_CB_database->setQuery( $query );
			if ( $_CB_database->query() ) {
				$query	=	'UPDATE ' . $_CB_database->NameQuote( '#__fb_users' )
	...
	static public function unsetSession( $userId ) {
		...
		$query		=	'SELECT *'
					.	"\n FROM " . $_CB_database->NameQuote( '#__fb_sessions' )
					.	"\n WHERE " . $_CB_database->NameQuote( 'userid' ) . " = " . (int) $userId;
		$_CB_database->setQuery( $query );
		$session	=	$_CB_database->loadResult();

		if ( $session ) {
			$query	=	'UPDATE ' . $_CB_database->NameQuote( '#__fb_sessions' )
	...
	static public function saveForum( $parentForum, $row, $user, $plugin ) {
		...
		if ( $parentForum ) {
			...
			if ( ! $forum ) {
				...
				$query				=	'SELECT *'
									.	"\n FROM " . $_CB_database->NameQuote( '#__fb_categories' )
			...
			if ( $forum ) {
				...
				$query				=	'UPDATE ' . $_CB_database->NameQuote( '#__fb_categories' )
									.	"\n SET " . $_CB_database->NameQuote( 'parent' ) . " = " . (int) $parentForum
									.	', ' . $_CB_database->NameQuote( 'name' ) . " = " . $_CB_database->Quote( $row->getName() )
									.	', ' . $_CB_database->NameQuote( 'pub_access' ) . " = 1"
									.	', ' . $_CB_database->NameQuote( 'pub_recurse' ) . " = 0"
									.	', ' . $_CB_database->NameQuote( 'admin_access' ) . " = 0"
									.	', ' . $_CB_database->NameQuote( 'admin_recurse' ) . " = 0"
		...
		if ( $forum ) {
			...
			if ( ! $_CB_database->query() ) {
				...
				if ( ! $_CB_database->query() ) {
					...
					$query			=	'SELECT ' . $_CB_database->NameQuote( 'id' )
									.	"\n FROM " . $_CB_database->NameQuote( '#__fb_messages' )
									.	"\n WHERE " . $_CB_database->NameQuote( 'catid' ) . " = " . (int) $forum;
					$_CB_database->setQuery( $query );
					...
					if ( $posts ) {
						...
						if ( ! $_CB_database->query() ) {
							...
							$query	=	'DELETE'
									.	"\n FROM " . $_CB_database->NameQuote( '#__fb_messages_text' )
	...
	static private function getItemid( $htmlspecialchars = false ) {
		...
		if ( ! isset( $Itemid ) ) {
			$query		=	'SELECT ' . $_CB_database->NameQuote( 'id' )
						...
						.	"\n WHERE " . $_CB_database->NameQuote( 'link' ) . " LIKE " . $_CB_database->Quote( 'index.php?option=com_kunena%' )
	...
	public function getTools( $msgs, $user, $plugin ) {
		...
		if ( $forum ) {
			...
			if ( $parentForum ) {
				...
				if ( ( $parentForum->pub_access != 1 ) || ( $parentForum->admin_access != 0 ) ) {
	...
	public function fixForum( $params, $user, $plugin ) {
		...
		if ( $params[0] == 'fixforums' ) {
			...
			if ( $forum ) {
				$query	=	'UPDATE ' . $_CB_database->NameQuote( '#__fb_categories' )
						.	"\n SET " . $_CB_database->NameQuote( 'pub_access' ) . " = 1"
						.	', ' . $_CB_database->NameQuote( 'pub_recurse' ) . " = 0"
						.	', ' . $_CB_database->NameQuote( 'admin_access' ) . " = 0"
						.	', ' . $_CB_database->NameQuote( 'admin_recurse' ) . " = 0"
