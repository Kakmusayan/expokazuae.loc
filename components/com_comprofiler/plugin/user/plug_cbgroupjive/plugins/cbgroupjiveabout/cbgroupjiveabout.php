	...
	private function saveAbout( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( cbgjClass::hasAccess( 'about_edit', $authorized ) ) {
			...
			if ( $plugin->params->get( 'about_editor', 3 ) >= 2 ) {
				$content	=	cbgjClass::getHTMLCleanParam( true, 'about_content', $params->get( 'about_content' ) );
			} else {
				$content	=	cbgjClass::getCleanParam( true, 'about_content', $params->get( 'about_content' ) );
			}
