...
class cbgjVideoPlugin extends cbPluginHandler {
	...
	public function getVideoApproval( $user, $plugin ) {
		...
		$where[]			=	array( 'published', '=', 0, 'c.params', 'CONTAINS', 'video_approve=1' );
			...
			foreach ( $rows as $row ) {
				...
				if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) ) {
					$return			.=						'<div class="gjDropdown">'
									...
									.								( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'video_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), null, true, false, null, true ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
				...
				if ( cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) {
					$return			.=					'<div class="gjContentBoxRow">'
									.						'<input type="button" value="' . htmlspecialchars( CBTxt::Th( 'Approve' ) ) . '" class="gjButton btn btn-success" onclick="' . cbgjClass::getPluginURL( array( 'plugin', 'video_publish', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ), true, true, false, null, true ) . '" />'
									.					'</div>';
				}
			...
			foreach ( $rows as $row ) {
				...
				if ( cbgjClass::hasAccess( array( 'mod_lvl4', 'mod_lvl3', 'usr_me' ), $authorized ) ) {
					$return		.=						'<div class="gjDropdown">'
								.							'<i class="icon-chevron-down"></i>'
								.							'<div class="gjDropdownItems">'
								.								( cbgjClass::hasAccess( array( 'mod_lvl3', 'usr_me' ), $authorized ) ? '<div><a href="' . cbgjClass::getPluginURL( array( 'plugin', 'video_edit', (int) $category->get( 'id' ), (int) $group->get( 'id' ), (int) $row->get( 'id' ) ) ) . '"><i class="icon-pencil"></i> ' . CBTxt::Th( 'Edit' ) . '</a></div>' : null )
	...
	private function saveVideo( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || cbgjClass::hasAccess( 'video_publish', $authorized ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'mod_lvl4', $authorized ), 'published', $row->get( 'published', ( $videoApprove  ? 0 : 1 ) ) ) );
			$row->set( 'user_id', (int) $row->get( 'user_id', $user->id ) );
			$row->set( 'group', (int) $row->get( 'group', $group->get( 'id' ) ) );
			$row->set( 'title', cbgjClass::getCleanParam( true, 'video_title', $row->get( 'title' ) ) );
			$row->set( 'url', cbgjClass::getCleanParam( true, 'video_url', $row->get( 'url' ) ) );
			$row->set( 'caption', cbgjClass::getCleanParam( true, 'video_caption', $row->get( 'caption' ) ) );
			...
			$notifications					=	cbgjData::getNotifications( array( array( 'video_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $group->get( 'id' ) ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'group_videonew=1', array( 'params', 'CONTAINS', 'group_videoupdate=1' ), array( 'params', 'CONTAINS', 'group_videoapprove=1' ) ) ) );
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'group_videoupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $videoApprove && ( ! cbgjClass::hasAccess( 'mod_lvl4', $authorized ) ) && $notificationParams->get( 'group_videoapprove' ) ) {
						$message			=	CBTxt::P( '[user] published [video_title_linked] in [group] and requires approval!', $row->getSubstitutionExtras( true ) );
					} elseif ( $notificationParams->get( 'group_videonew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::T( 'Video saved successfully!' ), false, true, null, false, false, true );
		} else {
			if ( $group->get( 'id' ) ) {
				$url						=	array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) );
			} elseif ( $category->get( 'id' ) ) {
...
class cbgjVideo extends comprofilerDBTable {
	...
	public function getCaption( $length = 0 ) {
		...
		if ( $caption ) {
			...
			if ( $length && ( strlen( strip_tags( $caption ) ) > $length ) ) {
				$caption	=	rtrim( trim( substr( strip_tags( $caption ), 0, $length ) ), '.' ) . '...';
			}
		}
