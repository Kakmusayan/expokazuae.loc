...
class CBplug_cbgroupjive extends cbPluginHandler {
	...
	private function sendOverviewMessage( $user, $plugin ) {
		...
		if ( cbgjClass::hasAccess( 'usr_mod', $authorized ) ) {
			$subject			=	cbgjClass::getCleanParam( true, 'subject' );
	...
	public function showCategoryEdit( $parent, $id, $user, $plugin, $message = null ) {
		...
		if ( $row->get( 'id' ) || ( ( ! $row->get( 'id' ) ) && ( ( ( ! $row->get( 'parent' ) ) && cbgjClass::hasAccess( 'cat_create', $authorized ) ) || ( $row->get( 'parent' ) && $row->getParentAccess( array( 'cat_nested_create', $user ) ) ) ) ) ) {
			...
			cbgjClass::getTemplate( 'category_edit' );
			...
			$categories					=	cbgjData::listArray( cbgjData::getCategories( array( array( 'cat_nested_create' ), $user ), array( 'id', '!=', (int) $row->get( 'id' ) ) ) );
	...
	private function saveCategoryEdit( $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || ( ( ! $row->get( 'id' ) ) && ( ( ( ! $row->get( 'parent' ) ) && cbgjClass::hasAccess( 'cat_create', $authorized ) ) || ( $row->get( 'parent' ) && $row->getParentAccess( array( 'cat_nested_create', $user ) ) ) ) ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( ( cbgjClass::hasAccess( 'usr_mod', $authorized ) || $row->getParentAccess( array( 'mod_lvl1', $user ) ) ), 'published', $row->get( 'published', ( $categoryApprove ? 0 : 1 ) ) ) );
			...
			$row->set( 'name', cbgjClass::getCleanParam( true, 'name', $row->get( 'name' ) ) );

			if ( $plugin->params->get( 'category_editor', 1 ) >= 2 ) {
				$row->set( 'description', cbgjClass::getHTMLCleanParam( true, 'description', $row->get( 'description' ) ) );
			} else {
			...
			$success_msg			=	CBTxt::P( '[category] saved successfully!', array( '[category]' => cbgjClass::getOverride( 'category' ) ) );

			$row->storeLogo( 'logo' );

			if ( $row->getError() ) {
				cbgjClass::displayMessage( $success_msg, 'message' );
			...
			if ( ! $row->get( 'parent' ) ) {
				$notifications		=	cbgjData::getNotifications( array( array( 'gen_usr_notifications' ), 'owner' ), array( array( 'type', '=', 'general' ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'general_categorynew=1', array( 'params', 'CONTAINS', 'general_categoryupdate=1' ), array( 'params', 'CONTAINS', 'general_categoryapprove=1' ) ) ) );
			} else {
				$notifications		=	cbgjData::getNotifications( array( array( 'cat_usr_notifications' ), 'owner' ), array( array( 'type', '=', 'category' ), array( 'item', '=', (int) $row->get( 'parent' ) ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'category_nestednew=1', array( 'params', 'CONTAINS', 'category_nestedupdate=1' ), array( 'params', 'CONTAINS', 'category_nestedapprove=1' ) ) ) );
			}
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'general_categoryupdate' ) || $notificationParams->get( 'category_nestedupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $categoryApprove && ( ( ! $row->get( 'parent' ) ) && ( ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) ) && $notificationParams->get( 'general_categoryapprove' ) ) || ( $row->get( 'parent' ) && ( ! $row->getParentAccess( array( 'mod_lvl1', $user ) ) ) && $notificationParams->get( 'category_nestedapprove' ) ) ) ) {
						$message	=	CBTxt::T( '[user] created [category] [category_override] and requires approval!' );
					} elseif ( $notificationParams->get( 'general_categorynew' ) || $notificationParams->get( 'category_nestednew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'categories', 'show', (int) $row->get( 'id' ) ), $success_msg, false, true, null, false, false, true );
		} else {
			cbgjClass::getPluginURL( array( 'overview' ), CBTxt::T( 'Not authorized.' ), false, true, 'error' );
		}
	}
	...
	private function joinGroup( $catid, $grpid, $id, $user, $plugin ) {
		...
		if ( $group->get( 'id' ) && ( ! $row->get( 'id' ) ) ) {
			...
			if ( $id ) {
				$where[]				=	array( 'code', '=', (int) $id );
			}

			$where[]					=	array( 'email', '=', $user->email, array( 'user', '=', (int) $user->id ) );
			$where[]					=	array( 'accepted', 'IN', array( '0000-00-00', '0000-00-00 00:00:00', '', NULL ) );

			$invites					=	cbgjData::getInvites( null, $where );
			...
			if ( $invites ) foreach ( $invites as $invite ) {
				...
				if ( ! $invite->store() ) {
					cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $group->get( 'id' ) ), CBTxt::P( 'Invite accept failed! Error: [error]', array( '[error]' => $invite->getError() ) ), false, true, 'error' );
				}
	...
	public function showGroupEdit( $catid, $parent, $id, $user, $plugin, $message = null ) {
		...
		if ( $row->get( 'id' ) || ( ( ! $row->get( 'id' ) ) && ( ( ( ! $row->get( 'parent' ) ) && cbgjClass::hasAccess( 'cat_grp_create', $authorized ) ) || ( $row->get( 'parent' ) && $row->getParentAccess( array( 'grp_nested_create', $user ) ) ) || ( ( ! $category->get( 'id' ) ) && cbgjClass::hasAccess( 'grp_create', $authorized ) ) ) ) ) {
			...
			cbgjClass::getTemplate( 'group_edit' );
			...
			$categories							=	cbgjData::getCategories( array( array( 'cat_grp_create' ), $user ) );

			if ( $categories ) {
				$input['category']				=	moscomprofilerHTML::selectList( cbgjData::listArray( $categories ), 'category', ( ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) && $category->get( 'id' ) ) ? 'disabled="disabled"' : null ) . ' class="input-large required"', 'value', 'text', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'usr_mod', $authorized ), 'category', $row->get( 'category', $category->get( 'id' ) ) ), 1, false, false );

				if ( $category->get( 'id' ) ) {
					$listGroups					=	cbgjData::listArray( cbgjData::getGroups( array( array( 'grp_nested_create' ), $user ), array( array( 'category', '=', (int) $category->get( 'id' ) ), array( 'id', '!=', (int) $row->get( 'id' ) ) ) ) );
				} else {
					$listGroups					=	array();

					foreach ( $categories as $cat ) {
						$groups					=	cbgjData::listArray( cbgjData::getGroups( array( array( 'grp_nested_create' ), $user ), array( array( 'category', '=', (int) $cat->get( 'id' ) ), array( 'id', '!=', (int) $row->get( 'id' ) ) ) ) );
				...
				if ( $listGroups ) {
					...
					$input['parent']			=	moscomprofilerHTML::selectList( $listGroups, 'parent', ( ( ! cbgjClass::hasAccess( 'usr_mod', $authorized ) && $row->get( 'id' ) ) ? 'disabled="disabled"' : null ) . ' class="input-large required"', 'value', 'text', (int) cbgjClass::getCleanParam( cbgjClass::hasAccess( 'usr_mod', $authorized ), 'parent', $row->get( 'parent', 0 ) ), 1, false, false );
				} else {
					if ( $category->get( 'id' ) ) {
				...
				$input['parent']				=	CBTxt::P( 'Please save [group] with [category] before selecting parent [group].', array( '[category]' => cbgjClass::getOverride( 'category' ), '[group]' => cbgjClass::getOverride( 'group' ) ) );
			}
	...
	private function saveGroupEdit( $catid, $id, $user, $plugin ) {
		...
		if ( $row->get( 'id' ) || ( ( ! $row->get( 'id' ) ) && ( ( ( ! $row->get( 'parent' ) ) && cbgjClass::hasAccess( 'cat_grp_create', $authorized ) ) || ( $row->get( 'parent' ) && $row->getParentAccess( array( 'grp_nested_create', $user ) ) ) || ( ( ! $category->get( 'id' ) ) && cbgjClass::hasAccess( 'grp_create', $authorized ) ) ) ) ) {
			...
			$row->set( 'published', (int) cbgjClass::getCleanParam( ( cbgjClass::hasAccess( 'mod_lvl1', $authorized ) || $row->getParentAccess( array( 'mod_lvl2', $user ) ) ), 'published', $row->get( 'published', ( $groupApprove ? 0 : 1 ) ) ) );
			...
			$success_msg			=	CBTxt::P( '[group] saved successfully!', array( '[group]' => cbgjClass::getOverride( 'group' ) ) );
			...
			if ( ! $row->get( 'parent' ) ) {
				$notifications		=	cbgjData::getNotifications( array( array( 'cat_usr_notifications' ), 'owner' ), array( array( 'type', '=', 'category' ), array( 'item', '=', (int) $category->get( 'id' ) ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'category_groupnew=1', array( 'params', 'CONTAINS', 'category_groupupdate=1' ), array( 'params', 'CONTAINS', 'category_groupapprove=1' ) ) ) );
			} else {
				$notifications		=	cbgjData::getNotifications( array( array( 'grp_usr_notifications' ), 'owner' ), array( array( 'type', '=', 'group' ), array( 'item', '=', (int) $row->get( 'parent' ) ), array( 'user_id', '!IN', array( (int) $user->id, (int) $row->get( 'user_id' ) ) ), array( 'params', 'CONTAINS', 'group_nestednew=1', array( 'params', 'CONTAINS', 'group_nestedupdate=1' ), array( 'params', 'CONTAINS', 'group_nestedapprove=1' ) ) ) );
			}
			...
			if ( $notifications ) foreach ( $notifications as $notification ) {
				...
				if ( ! $new ) {
					if ( $notificationParams->get( 'category_groupupdate' ) || $notificationParams->get( 'group_nestedupdate' ) ) {
					...
					if ( ( $row->get( 'published' ) == 0 ) && $groupApprove && ( ( ! $row->get( 'parent' ) ) && ( ( ! cbgjClass::hasAccess( 'mod_lvl1', $authorized ) ) && $notificationParams->get( 'category_groupapprove' ) ) || ( $row->get( 'parent' ) && ( ! $row->getParentAccess( array( 'mod_lvl2', $user ) ) ) && $notificationParams->get( 'group_nestedapprove' ) ) ) ) {
						$message	=	CBTxt::T( '[user] created [group] in [category] and requires approval!' );
					} elseif ( $notificationParams->get( 'category_groupnew' ) || $notificationParams->get( 'group_nestednew' ) ) {
			...
			cbgjClass::getPluginURL( array( 'groups', 'show', (int) $category->get( 'id' ), (int) $row->get( 'id' ) ), $success_msg, false, true, null, false, false, true );
		} else {
			cbgjClass::getPluginURL( array( 'overview' ), CBTxt::T( 'Not authorized.' ), false, true, 'error' );
		}
	}
