	...
	static public function showActivityTab( $rows, $ajax, $pageNav, $viewer, $user, $plugin ) {
		...
		if ( ! $ajax ) {
			$return				.=	'<div class="activityTab">'
								.		'<form action="' . ( $ajaxPaging ? cbactivityClass::getTabClassURL( $user->id, true, 'raw' ) : $_CB_framework->userProfileUrl( $user->id, true, $plugin->tab->tabid ) ) . '" method="post" name="activityForm" id="activityForm" class="activityForm">';
		}

		if ( $rows ) foreach ( $rows as $row ) {
			$message			=	HTML_cbactivityActivity::getMessageDisplay( $row, $cache );
			...
			$return				.=			'<div id="activity' . (int) $row->get( 'id' ) . '" class="activityContent' . htmlspecialchars( $row->get( 'class' ) ) . ' row-fluid">'
								...
								.						'<div class="activityContentBodyTitle span9"><h5>' . $row->getOwnerName( true ) . ( $row->get( 'title' ) ? ' <small>' . $row->getTitle() . '</small>' : null ) . '</h5></div>'
