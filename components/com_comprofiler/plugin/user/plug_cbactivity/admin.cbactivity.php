...
class cbactivityAdmin extends cbPluginHandler {
	...
	private function saveActivityEdit( $id, $task, $user, $plugin ) {
		...
		if ( $row->getError() || ( ! $row->store() ) ) {
			return $this->showActivityEdit( $id, $user, $plugin, CBTxt::P( 'Activity failed to save! Error: [error]', array( '[error]' => $row->getError() ) ) );
		}
	...
	private function saveConfig( $config, $user, $plugin ) {
		...
		if ( $row->getError() || ( ! $row->store() ) ) {
			return $this->showConfig( $user, $plugin, CBTxt::P( 'Config failed to save! Error: [error]', array( '[error]' => $row->getError() ) ) );
		}
