...
class cbactivityClass {
	...
	static public function getPluginURL( $variables = array(), $msg = null, $htmlspecialchars = true, $redirect = false, $type = null, $return = false, $ajax = false, $back = false ) {
		...
		if ( $msg ) {
			if ( $redirect ) {
				cbactivityClass::setRedirect( $url, $msg, $type );
			} else {
				if ( $msg === true ) {
	...
	static public function getTemplate( $files = null, $loadHeader = true ) {
		global $_CB_framework;

		static $tmpl							=	array();

		$id										=	cbactivityClass::getStaticID( $files, $loadHeader );

		if ( ! isset( $tmpl[$id] ) ) {
			$plugin								=	cbactivityClass::getPlugin();
			$template							=	$plugin->params->get( 'general_template', 'default' );
			$files								=	( ! is_array( $files ) ? array( $files ) : $files );
			$paths								=	array( 'global_css' => null, 'php' => null, 'css' => null, 'js' => null );
	...
	public function getTitle( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			if ( $this->title ) {
				...
				$cache[$id]			=	CBuser::getInstance( $this->get( 'user_id' ) )->replaceUserVars( CBTxt::P( $this->get( 'title' ), $substitutions ) );
			} else {
				$cache[$id]			=	null;
			}
		}
		...
		if ( $title ) {
			...
			if ( $length && ( strlen( strip_tags( $title ) ) > $length ) ) {
				$title				=	rtrim( trim( substr( strip_tags( $title ), 0, $length ) ), '.' ) . '...';
			}
		}
	...
	public function getMessage( $length = 0 ) {
		...
		if ( ! isset( $cache[$id] ) ) {
			if ( $this->message ) {
				...
				$cache[$id]			=	CBuser::getInstance( $this->get( 'user_id' ) )->replaceUserVars( CBTxt::P( $this->get( 'message' ), $substitutions ) );
			} else {
				$cache[$id]			=	null;
			}
		}
		...
		if ( $message ) {
			...
			if ( $length && ( strlen( strip_tags( $message ) ) > $length ) ) {
				$message			=	rtrim( trim( substr( strip_tags( $message ), 0, $length ) ), '.' ) . '...';
			}
		}
...
class cbactivityData {
	...
	static public function getActivity( $filtering = array(), $ordering = array(), $limits = 0, $list = true ) {
		global $_CB_framework, $_CB_database;
