...
class CBplug_cbactivity extends cbPluginHandler {
	...
	public function getCBpluginComponent( $tab, $user, $ui, $postdata ) {
		...
		if ( ! $ajax ) {
			...
			cbactivityClass::getTemplate( array( 'recent', 'activity' ), false );
		}
		...
		switch( $display ) {
			case 2: // Connections Only
				$where				=	array( 'b.referenceid', '=', (int) $user->get( 'id' ) );
				break;
			case 3: // Self Only
				$where				=	array( 'user_id', '=', (int) $user->get( 'id' ) );
				break;
			case 4: // Connections and Self
				$where				=	array( 'user_id', '=', (int) $user->get( 'id' ), array( 'b.referenceid', '=', (int) $user->get( 'id' ) ) );
				break;
			default: // Everyone
				$where				=	array();
				break;
