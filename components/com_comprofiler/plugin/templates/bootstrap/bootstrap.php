...
class CBProfileView_html_bootstrap extends cbProfileView {
	...
	public function _render() {
		...
		$menu_profile = JModuleHelper::getModules('menu_profile');
		$return				.=		'<div class="cbPosLeft ' . $spanClass . '">'.$menu_profile[0]->content.'</div>';		
		
		$return				.=		'<div class="cbPosRightContent">';	
		...
		if ( isset( $this->userViewTabs['cb_head'] ) ) {
 		...
 		if ( $this->nCols != 0 ) {
			$return					.=	'<div class="cbPosTop">';

			 if ( $this->nCols > 2 ) {
				$spanClass			=	'cbPosTopThird';
			 } elseif ( $this->nCols > 1 ) {
				$spanClass			=	'cbPosTopHalf';
			 } else {
				$spanClass			=	'cbPosTopFull';
			 }
			 
			//if ( isset( $this->userViewTabs['cb_left'] ) ) {
				$return				.=		'<div class="cbPosLeft ' . $spanClass . '"></div>';
			//}

			if ( isset( $this->userViewTabs['cb_middle'] ) ) {
				if($_GET['id']!='subscribe'){
				$return				.=		'<div class="cbPosMiddle ' . $spanClass . '">'
			...
			if ( isset( $this->userViewTabs['cb_right'] ) ) {
				$return				.=		'<div class="cbPosRight ' . $spanClass . '">'
		...
		if ( $indexes ) foreach ( $indexes as $k => $v ) {
			if ( $v && $v[0] == 'L' ) {
				...
				if ( ! ( isset( $indexes[$k + 1] ) && ( $indexes[$k + 1][1] == $L ) ) ) {
					...
					$return			.=	'<div class="cbPosGridLine" id="cbPosLine' . substr( $v, 0, 2 ) . '">';
					...
					for ( $i = $line ; $i <= $k ; $i++ ) {
						...
						$return		.=	'<div class="cbPosGrid" id="cbPos' . $v . '_' . $i . '" style="width:' . (int) $step . '%;">'
	...
	public function _renderEdit() {
		...
		$_CB_framework->outputCbJQuery( "$( '.cbEditProfile' ).parent().addClass( '' )" );
